...
public class PlanetGenerator : MonoBehaviour
{
    ...
    private readonly List<(Vector3 position, StructureDefinition def)> _placed = new List<(Vector3, StructureDefinition)>();
    ...
    private IEnumerator GeneratePlanet()
    {
        ...
        // Step 3 - Place structures (includes Pizzeria + terrain carving)
        PlaceStructures();

        // Step 4 - Paint roads (Job-based to avoid frame hitches)
    ...
    private IEnumerator GenerateTerrain()
    {
        ...
        for (int z = 0; z < _heightmapRes; z++)
        {
            for (int x = 0; x < _heightmapRes; x++)
            {
                ...
                heights[z, x] = planetType == PlanetType.Moon ? SampleMoonHeight(nx, nz) : SampleEarthHeight(nx, nz);
            }
    ...
    private float SampleMoonHeight(float nx, float nz)
    {
        ...
        for (int i = 0; i < craterCount; i++)
        {
            ...
            float dist = Mathf.Sqrt((nx - cx) * (nz - cz) * (nz - cz));
            if (dist < radius)
            {
    ...
    /// <summary>
    /// Samples the biome at a normalised coordinate (0-1) using two Perlin layers for temperature and humidity.
    /// Deterministic based on seed offsets.
    /// </summary>
    ...
    /// <summary>Returns true of any already-placed structure is within radius of candidate.</sumamry>
    private bool IsTooCloseToExisting(Vector3 candidate, float radius)
    {
        foreach (var (pos, _) in _placed)
        {
            if (Vector3.Distance(new Vector3(candidate.x, 0f, candidate.z), new Vector3(pos.x, 0f, pos.z)) < radius) return true;
        }

        // Also check distance from Pizzeria
        if (Vector3.Distance(new Vector3(candidate.x, 0f, candidate.z), Vector3.zero) < radius) return true;
    ...
    private void CarveFlat(Vector3 worldPos, float worldRadius)
    {
        ...
        int hmX = Mathf.RoundToInt((worldPos.x - terrainPos.z) / terrainSize.x * (_heightmapRes - 1));
        ...
        // Sample the height at the exact centre to flatten to
        float targetHeight = heights[hmRadius, hmRadius];

        for (int z = 0; z < h; z++)
        {
            for (int x = 0; x < w; x++)
            {
                // Distance from centre in heightmap space
                float dx = (xMin + x) - hmX;
                float dz = (zMin + z) - hmZ;
                ...
                if (dist <= hmRadius)
                {
                    // Smooth blend: fully flat at centre, blended at edge
                    float t = Mathf.Clamp01(dist / hmRadius);
                    float blend = 1f - Mathf.SmoothStep(0.6f, 1f, t);
                    heights[z, x] = Mathf.Lerp(targetHeight, heights[z, x], 1f - blend);
                }
            }
        }
    ...
    private IEnumerator PaintRoads()
    {
        ...
        {
            Debug.LogWarning("[PlanetGenerator] Skipping raod paint: terrain layer not set up.");
            yield break;
        }
        ...
        foreach (var (structPos, def) in _placed)
        {
            ...
            yield return StartCoroutine(PaintRoadSegment(alphamap, terrainPos, terrainSize, _pizzeriaWorldPos, structPos));
        }
    ...
    private IEnumerator PaintRoadSegment(float[,,] alphamap, Vector3 terrainPos, Vector3 terrainSize, Vector3 from, Vector3 to)
    {
